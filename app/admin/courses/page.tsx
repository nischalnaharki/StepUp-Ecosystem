...
export default async function Courses({ searchParams }: { searchParams: Promise<{ error?: string; created?: string; updated?: string; deleted?: string }> }) {
  ...
  const [courses, params] = await Promise.all([
    prisma.course.findMany({ include: { _count: { select: { students: true } }, bookAsset: { select: { id: true } } }, orderBy: { createdAt: "asc" } }),
    searchParams,
  ]);
