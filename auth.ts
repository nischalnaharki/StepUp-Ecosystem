...
async function hasValidAdminVerification(
  admin: {
    luckyNumberHash: string | null;
    dobBsHash: string | null;
    favoriteColorHash: string | null;
    currentCollegeHash: string | null;
  },
  ...
  const hashes = [
    admin.luckyNumberHash,
    admin.dobBsHash,
    admin.favoriteColorHash,
    admin.currentCollegeHash,
  ];
  ...
  const expected = [
    process.env.ADMIN_LUCKY_NUMBER,
    process.env.ADMIN_DOB_BS,
    process.env.ADMIN_FAVORITE_COLOR,
    process.env.ADMIN_CURRENT_COLLEGE,
  ];
  ...
  return expected.every((answer, index) => {
    const providedAnswers = [
      ...
      answers.currentCollege,
    ];
    return Boolean(answer) && providedAnswers[index] === normalizedAnswer(answer);
  });
}
  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(c) {
        ...
        // A student may have joined with Google and set a password later.
        // Login eligibility for this provider is deliberately based only on
        // passwordHash: googleId is an alternate sign-in method, not a
    ...
    Credentials({
      ...
      async authorize(c) {
        const admin = await prisma.admin.findUnique({
          where: { email: String(c?.email ?? "").trim().toLowerCase() },
        });

        if (
          !admin ||
          !(await bcrypt.compare(String(c?.password ?? ""), admin.passwordHash)) ||
          !(await hasValidAdminVerification(admin, c))
        ) {
          return null;
        }
  ...
  callbacks: {
    async signIn({ user, account }) {
      if (account?.provider === "google") {
        ...
        user.id = existing.id;
        user.role = "student";
        user.status = existing.approvalStatus;
        user.course = existing.courseId;
      }
    ...
    async jwt({ token, user }) {
      ...
      if (token.role === "student") {
        ...
        if (!student || !token.sessionId || student.activeSessionId !== token.sessionId) {
          return null;
        }
      }
    ...
    async session({ session, token }) {
      ...
      session.user.course = token.course as string | undefined;
      return session;
    },
  },
