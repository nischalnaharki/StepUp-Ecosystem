...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [
    ...
    Credentials({ id: "student-credentials", name: "Student login", credentials: { email: {}, password: {} }, async authorize(c) {
      ...
      return { id: student.id, name: student.name, email: student.email, role: "student", status: student.approvalStatus, course: student.selectedCourse };
    }}),
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      ...
      if (!existing) {
        const course = (await cookies()).get("stepup-google-course")?.value as Course | undefined;
        if (!course || !Object.values(Course).includes(course)) return "/register?error=google-course";
        existing = await prisma.student.create({ data: { name: user.name || "StepUp Student", email, googleId: account.providerAccountId, selectedCourse: course } });
      }
      user.id = existing.id; user.role = "student"; user.status = existing.approvalStatus; user.course = existing.selectedCourse;
      return true;
    },
