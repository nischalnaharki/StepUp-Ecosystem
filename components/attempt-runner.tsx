...
export function AttemptRunner({
    ...
    try {
      const response = await fetch(`/api/attempts/${attemptId}`, {
        ...
        body: JSON.stringify({
          ...
          flaggedQuestionIds: flagsRef.current,
          timeRemainingSeconds: remainingRef.current,
          ...extra,
        }),
      });
  ...
  async function finish(auto = false) {
    ...
    const flagged = flagsRef.current.length;

    if (
      !auto &&
      !window.confirm(
        `Submit now? ${unanswered} unanswered and ${flagged} flagged question(s) will be submitted as they are.`,
      )
    ) {
      return;
    }
  ...
  return (
    <main className="course attempt-page">
      <p className="eyebrow">MOCK TEST · {testName}</p>
      {remaining !== null && <p className="timer">Time remaining: {formatTime(remaining)}</p>}
      ...
      <div className="attempt-layout">
        ...
        <section className="card question-screen">
          <p className="fine">
            {question.sectionName} · Question {index + 1} of {questions.length}
          </p>
          <h1>{question.text}</h1>

          <div className="answer-options">
            {question.optionOrder.map((originalIndex, optionIndex) => (
              <label key={originalIndex} className={answers[question.id] === originalIndex ? "selected" : ""}>
                <input
                  type="radio"
                  name="answer"
