...
export function BookViewer() {
  ...
  const savePage = useCallback((page: number, mark?: number | null) => {
    void fetch("/api/book/progress", {
      ...
      body: JSON.stringify({
        page,
        ...(mark !== undefined ? { markedPage: mark } : {}),
      }),
      keepalive: true,
    });
  }, []);
  ...
  useEffect(() => {
    ...
    async function render() {
      try {
        ...
        if (response.status === 403 || progressResponse.status === 403) {
          window.location.assign("/login?error=signed-in-elsewhere");
          return;
        }
        ...
        const validMarkedPage =
          Number.isInteger(savedMark) && savedMark >= 1 && savedMark <= pdf.numPages
            ? savedMark
            : null;
        ...
        for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber++) {
          ...
          if (!cancelled) {
            pages.current.push(pageElement);
            container.current?.append(pageElement);
          }
        }
    ...
    return () => {
      cancelled = true;
      observer?.disconnect();
      savePage(currentPageRef.current);
    };
  ...
  return (
    <section className="book-viewer" onContextMenu={(event) => event.preventDefault()}>
      {totalPages > 0 && (
        <div className="book-navigation" aria-label="Book navigation">
          ...
          <form onSubmit={submitPage}>
            ...
            <input
              ...
              aria-label={`Current page, of ${totalPages}`}
            />
          ...
          <label className="book-mark">
            <input
              type="checkbox"
              checked={markedPage === currentPage}
              onChange={(event) => toggleMark(event.target.checked)}
            />{" "}
            Mark page
          </label>

          {markedPage && markedPage !== currentPage && (
            <button type="button" className="secondary small" onClick={() => goToPage(markedPage)}>
              Marked page {markedPage}
            </button>
          )}
