...
export function MockTestList({ tests }: { tests: AvailableTest[] }) {
  ...
  return <section className="mock-test-list">{message && <p className="error">{message}</p>}{tests.map((test) => <article key={test.id} className="course-card"><div><span>TEST</span><h2>{test.name}</h2><p>{test.questionCount} questions · {test.timeLimitMinutes ? `${test.timeLimitMinutes} minutes` : "Untimed"}</p><p className="fine">{test.status === "NOT_STARTED" ? "Not started" : test.status === "IN_PROGRESS" ? "In progress" : "Completed"}</p></div><button className="button" disabled={busy === test.id} onClick={() => start(test.id)}>{busy === test.id ? "Opening…" : test.status === "IN_PROGRESS" ? "Resume" : test.status === "COMPLETED" ? "Retake" : "Start"}</button></article>)}</section>;
}
